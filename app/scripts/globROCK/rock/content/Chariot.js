...
var self = require( "globROCK/rock/RockContent" )( $core, $control );
...
self.awake = function( rock ){
  var max = rock.toonList.length;
  var toonList = rock.toonList;
  ...
  if ( self.core.parent == null ){
    trace("add to soul layer ");
    self.core.glob.soulLayer.addChild( display );
  } 

  if ( self.core.parent != null ){
    trace("adding to parent");
   self.core.parent.display.addChild( display );
  }
...
self.buildToon = function( toon  ){
  ...
  for ( var i = 0; i < max; i++ )
  {
    ...
    if (index == 'x') return;
    if ( PIXI == null) continue;
    if ( PIXI.Texture == null ) continue;
  ...
  display.onComplete = function() { 
    var soul = self.core.soul;
    if ( toon.end != null ) soul[ toon.end ]( self.core.rock );
  }; 
  };
